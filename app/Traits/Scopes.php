...
trait Scopes
{
    ...
    public function scopeWhereHasNotCurrentMonth(
        Builder $query,
        string $relation,
        ...
        return $query->whereHas($relation, function ($q) use ($currentYear, $currentMonth, $yearColumn, $monthColumn) {
            $q->where(function ($q) use ($currentYear, $currentMonth, $yearColumn, $monthColumn) {
                $q->where($yearColumn, '<', $currentYear)
                    ->orWhere($yearColumn, '>', $currentYear)
                    ->orWhere(function ($q) use ($currentYear, $currentMonth, $yearColumn, $monthColumn) {
                        $q->where($yearColumn, $currentYear)
                            ->where($monthColumn, '!=', $currentMonth);
                    });
            });
        });
    }
