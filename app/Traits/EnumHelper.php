...
trait EnumHelper
{
    ...
    public static function valuesExcept(array $except): array
    {
        return array_values(
            array_filter(
                self::values(),
                fn ($value) => ! in_array($value, $except)
            )
        );
    }

    public static function valuesOnly(array $only): array
    {
        return array_values(
            array_filter(
                self::values(),
                fn ($value) => in_array($value, $only)
            )
        );
    }
}
