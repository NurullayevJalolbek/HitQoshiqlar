    ...
    <div class="settings-card">
        ...
        <div class="dual-select">
            <div class="select-wrapper">
                <label class="section-label">Davlat</label>
            ...
            <div class="select-wrapper">
                <label class="section-label">Vaqt mintaqasi (Timezone)</label>
                <select class="form-select" id="timezoneSelect"></select>
                <span class="form-hint">Avtomatik aniqlanadi</span>
            </div>
        </div>
        ...
        <div class="row-layout">
            ...
            <div class="form-section">
                ...
                <div class="custom-input-wrapper" id="customDateInput">
                    <input type="text" class="custom-input" placeholder="Masalan: d.m.Y yoki Y-m-d" id="customDateFormat">
                </div>
            ...
            <div class="form-section">
                ...
                <div class="custom-input-wrapper" id="customTimeInput">
                    <input type="text" class="custom-input" placeholder="Masalan: H:i:s yoki g:i A" id="customTimeFormat">
                </div>
        ...
        <div class="row-layout">
            {{-- Til --}}
            <div class="form-section">
                <label class="section-label">Tizimning asosiy tili</label>
                <select class="form-select" id="languageSelect">
                    <option value="uz" selected>🇺🇿 O'zbek tili (uz)</option>
                    <option value="en">🇬🇧 Ingliz tili (en)</option>
                    <option value="ru">🇷🇺 Rus tili (ru)</option>
                    <option value="ar">🇸🇦 Arab tili (ar)</option>
                </select>
                <span class="form-hint">Interfeys va xabarlar uchun til</span>
            </div>

            {{-- Valyuta --}}
            <div class="form-section">
                <label class="section-label">Valyuta</label>
                <select class="form-select" id="currencySelect">
                    <option value="uzs" selected>🇺🇿 UZS – O'zbekiston so'mi</option>
                    <option value="usd">🇺🇸 USD – AQSh dollari</option>
                    <option value="eur">🇪🇺 EUR – Yevro</option>
                    <option value="rub">🇷🇺 RUB – Rossiya rubli</option>
                    <option value="aed">🇦🇪 AED – BAA dirhami</option>
                </select>
                <span class="form-hint">Narxlar va to'lovlar uchun valyuta</span>
            </div>
        </div>
...
<script>
    ...
    const timezones = {
        ...
        ru: ['Europe/Moscow (UTC+3)', 'Asia/Yekaterinburg (UTC+5)', 'Asia/Novosibirsk (UTC+7)', 'Asia/Vladivostok (UTC+10)'],
        us: ['America/New_York (UTC-5)', 'America/Chicago (UTC-6)', 'America/Denver (UTC-7)', 'America/Los_Angeles (UTC-8)'],
        ae: ['Asia/Dubai (UTC+4)']
    };
    ...
    // Update timezones based on country
    ...
    document.querySelectorAll('.format-option').forEach(option => {
        option.addEventListener('click', function() {
            ...
            const isDateOption = this.closest('.form-section').querySelector('.section-label').textContent.includes('Sana');
            const isTimeOption = this.closest('.form-section').querySelector('.section-label').textContent.includes('Vaqt');
            ...
            if (isDateOption) {
                ...
                if (value === 'custom') {
                    ...
                    document.getElementById('datePreview').textContent = document.getElementById('customDateFormat').value || 'Custom format';
                } else {
                ...
                if (value === 'custom') {
                    customInput.classList.add('active');
                    document.getElementById('timePreview').textContent = document.getElementById('customTimeFormat').value || 'Custom format';
                } else {
                    customInput.classList.remove('active');
