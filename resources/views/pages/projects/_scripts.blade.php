    ...
    <script>
        ...
        function toggleRisksListEdit() {
            ...
            if (risksListEditMode) {
                originalRiskItems = JSON.parse(JSON.stringify(projectData.risks.risk_items || []));

                btn.innerHTML = '<i class="bi bi-check-lg me-1"></i> Saqlash';
                btn.classList.remove('btn-outline-secondary');
                btn.classList.add('btn-success');
            } else {
                saveRisksListChanges();
        ...
        function displayRisks(risks) {
            ...
            /* ---------- INFO CARD ---------- */
            const info = document.getElementById('risksInfoContent');

            if (!risksInfoEditMode) {
                info.innerHTML = `
                <div class="info-row">
                    <span class="info-label">Boshqaruv modeli</span>
                    <span class="info-value">${uiEscapeHtml(risks.management_model || '-')}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Tavsif</span>
                    <span class="info-value">${uiEscapeHtml(
                    risks.management_description ||
                    risks.management_desc ||
                    risks.management_info ||
                    '-'
                )}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Xatar darajasi</span>
                    <span class="info-value">${uiEscapeHtml(risks.risk_level || '-')}</span>
                </div>
            `;
            } else {
                info.innerHTML = `
                <div class="info-row">
                    <span class="info-label">Boshqaruv modeli</span>
                    <input class="form-control"
                           value="${uiEscapeHtml(risks.management_model || '')}"
                           onchange="projectData.risks.management_model = this.value">
                </div>
                <div class="info-row">
                    <span class="info-label">Tavsif</span>
                    <textarea class="form-control" rows="3"
                        onchange="projectData.risks.management_description = this.value">
    ${uiEscapeHtml(
                    risks.management_description ||
                    risks.management_desc ||
                    risks.management_info ||
            ...
            if (!Array.isArray(risks.risk_items) || risks.risk_items.length === 0) {
                list.innerHTML = '<p class="text-muted text-center py-4">Xatarlar mavjud emas</p>';
                return;
            }

            if (!risksListEditMode) {
                list.innerHTML = risks.risk_items.map(r => `
                <div class="risk-item">
            ...
            `).join('');
                ...
                <div class="risk-item">
                    <input class="form-control mb-2"
                        ...
                        onchange="r.name = this.value">
                    <textarea class="form-control" rows="3"
                        onchange="r.description = this.value">${uiEscapeHtml(r.description)}</textarea>
                </div>
            `).join('');
            }
        }
